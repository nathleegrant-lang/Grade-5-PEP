...
const neutralMessage = "If that email is eligible for confirmation, we’ve sent a new message. Please check your inbox and spam/junk folder."
...
export default function ResendConfirmationPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (error) console.error("Confirmation resend request failed:", error)
      setMessage(neutralMessage)
    } catch (error) {
      console.error("Unexpected confirmation resend error:", error)
      setMessage(neutralMessage)
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0d4a5f] to-[#0d9488] p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl text-[#1e3a5f]">Resend Confirmation Email</CardTitle>
          <CardDescription>Enter the email address you used and we’ll send the next step where eligible.</CardDescription>
        </CardHeader>
        <CardContent>
